...
- **`npm run export:bok`** (`scripts/export-bok.mjs`) — läsfärdig bok till `dist/bok/teknik-niva-1-och-2.docx`, byggd ur samma sammanställning som de två andra. Skillnaden är att detta är en bok och inte ett manus: titelsida, innehållsförteckning som Word-fält, förord, kapitel med sidbrytning före varje kapitel, facit sist och baksidestext. Sidnumreringen kommer ur sidfoten i `scripts/review-reference.docx`, som återanvänds. Det enda produktionsledet som följer med är bildplatshållarna, eftersom illustrationerna ännu inte är producerade; frånvaron av övrig metadata kontrolleras i efterkontrollerna. För- och eftertexterna (titelsida, förord, baksidestext) ligger i `bokdelar/` och är fri redigeringsyta utanför `content/`, som bara rymmer kursplanestyrt innehåll. Sidbrytnings- och TOC-fältstransformationen görs av Pandoc-filtret `scripts/bok-filter.lua`.
