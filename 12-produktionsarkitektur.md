...
Fälten `module`, `uppslag` och `practical_component` är **pensionerade** (redaktionellt beslut). Modul fanns bara som mellannivå i den tidigare, nu avvecklade arkitekturen. Uppslag är en layoutfråga, och praktisk täckning härleds från avsnittens Övningar, inte från en boolesk flagga.
...
Varje avsnittssida inleds med en **granskningsruta** (genererad ur frontmattern av `site/src/remark-granskning.mjs`): lärandemål, status, kursplanetaggar, begrepp, figurer och förkunskaper. Samma plugin byter stycken som inleds med `[BILD X.Y-N]` mot `<aside class="bildplatshallare">` med etikett, Innehåll och Bildtext, så att bildplatshållare går att skilja från löptext i produktionsvyn. Källformatet är oförändrat och exporten påverkas inte; ändras klassnamnet måste rensningen i `site/src/pages/review/_data.mjs` följa med, annars felar bygget. Rutan finns bara i webbvyn och hamnar aldrig i exporten, som läser källfilerna direkt. Samma plugin renderar `[[begrepp:...]]` som länk till huvudstället och `[[figur:...]]` som platshållarruta ur registret; okänt begrepp eller okänd figur ger fel i både validate och bygget. Detta administrativa produktionsläge är den ordinarie webbplatsens beteende och lämnas oförändrat — klickbara begreppslänkar hör hemma i ett internt granskningsverktyg (se "Länkar i elevtext" nedan).
...
**Facit ligger sist även i granskningsvyn.** Facitfilerna lagras hos sina kapitel men plockas ur kapitlen i byggsteget (`_data.mjs`, `hamtaReview()`) och samlas på en egen sida, `/review/facit/`, med ett block per kapitel. Vyn speglar därmed exportens läsordning i `scripts/manuscript-core.mjs`, som skriver ut `# Facit` efter sista kapitlet. Facit visas alltså aldrig inne i en kapitelsida; sista kapitlets framåtlänk pekar på facit, och i `/review/hela-manuset/` står facitdelen efter sista kapitlet.

Granskningsvyn har en permanent vänsterspalt (desktop) som visar hela manusets hierarki — kapitel, avsnitt och facit sist — genererad ur samma bokstruktur (06 via `bokstruktur-data.mjs`) som resten av produktionen, med aktuell sida markerad och rätt gren expanderad. Spalten är egen för `/review/` (`site/src/pages/review/_ReviewLayout.astro`) och påverkar inte den ordinarie webbplatsens Starlight-sidopanel, som är oförändrad (se "Navigering"). På smal skärm ersätts spalten av en öppningsbar meny.
...
Det finns ingen `type: uppgiftsbank` längre (redaktionellt beslut 2026-07-22). Kapitel 11:s elevdrivna projekt (redaktionellt beslut 2026-07-29) får ingen egen `type` och ingen egen fil, utan skrivs i kapitlets avsnittsfiler (03, "Elevdrivna projekt och kapitelövergripande uppgifter"). Befintliga projektuppgiftsfiler avvecklas kapitel för kapitel i innehållsmigreringen och raderas.
...
**Elevfacit** är svar på instuderingsfrågor och övningar. Det ingår i boken som ett eget kapitel sist (03-bokens-arkitektur.md, "Facit") och ligger därmed i klartext i detta repo, i elevmanuset (`npm run export`) och på elevwebben.

**Facit lagras per kapitel, inte samlat.** Källfilen är kapitlets tredje kapitelavslutning (`type: facit`, `<NN>-facit.md`, efter sammanfattning och begreppslista), och `scripts/manuscript-core.mjs` samlar blocken och placerar dem sist i den sammanställda läsordningen under rubriken "Facit". Skälet är att ett facit är en spegling av kapitlets övningar och glider tyst isär från dem om det lagras långt bort. Per kapitel skrivs, committas och låses facit tillsammans med kapitlet, och `scripts/validate.mjs` kan stämma av varje uppgiftsnummer mot avsnittsfilerna i samma kapitel. Uppgifter som medvetet saknar svar registreras i frontmatterfältet `utanSvar` så att de går att skilja från glömda. Facitraden i 06-bokstruktur.md är valfri och läggs till kapitel för kapitel, eftersom facit inte kan skrivas förrän kapitlets övningar är klara. Eftersom repot är **offentligt** (GitHub Pages) blir elevfacit publikt läsbart. Det är ett accepterat beslut av projektägaren, inte en förbisedd följd: facit är avsett för eleven, och boken ska kunna läsas utan lärare, vilket förutsätter att eleven kan kontrollera sitt eget svar.
...
- **`npm run export:bok`** (`scripts/export-bok.mjs`) — läsfärdig bok till `dist/bok/teknik-niva-1-och-2.docx`, byggd ur samma sammanställning som de två andra. Skillnaden är att detta är en bok och inte ett manus: titelsida, innehållsförteckning som Word-fält, förord, kapitel med sidbrytning före varje kapitel, facit sist och baksidestext. Typografin kommer ur ett eget referensdokument, `scripts/bok-reference.docx` (byggs av `scripts/make-bok-reference.mjs`, redigeras aldrig för hand): titelsida, kapitel- och avsnittsrubriker i bokens mörkblå, sidfot med sidnummer och ingen siffra på omslaget. Manusexporternas `review-reference.docx` är avsiktligt kvar som ett manusutseende. Zip- och stilhanteringen delas av de två referensskripten via `scripts/docx-zip.mjs`.
- **`npm run export:pdf`** (`scripts/export-pdf.mjs`) — samma bok som PDF till `dist/bok/teknik-niva-1-och-2.pdf`. Kör först bokexporten med alla dess efterkontroller och konverterar därefter med Word via COM-automation från PowerShell. Word används därför att maskinen varken har LaTeX eller LibreOffice, och därför att det är den vägen som kan räkna om innehållsförteckningens fält så att den får riktiga sidnummer. Dokumentet öppnas osynligt och stängs utan att sparas, så docx-filen på disk är oförändrad efteråt; är den öppen i Word avbryts exporten med ett tydligt fel. Egna efterkontroller: giltig PDF-signatur och slutmarkör, rimligt sidantal, rubriker som PDF-bokmärken. Det enda produktionsledet som följer med är bildplatshållarna, eftersom illustrationerna ännu inte är producerade; frånvaron av övrig metadata kontrolleras i efterkontrollerna. För- och eftertexterna (titelsida, förord, baksidestext) ligger i `bokdelar/` och är fri redigeringsyta utanför `content/`, som bara rymmer kursplanestyrt innehåll. Sidbrytnings- och TOC-fältstransformationen görs av Pandoc-filtret `scripts/bok-filter.lua`.
