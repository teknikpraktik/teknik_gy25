---
id: "2.1"
chapter: 2
sectionNumber: 1
title: Vad är ett tekniskt problem?
status: fardig-forsta-version
levels: [niva1]
curriculumReferences:
  niva1: ["n1-04"]
  niva2: []
learningGoals:
  - Förklara vad som kännetecknar ett tekniskt problem och skilja mellan behov,
    problem och lösning.
  - Formulera ett tekniskt problem med funktion, krav och begränsningar.
abilities: [f2]
concepts_introduced: ["tekniskt problem", "funktion", "krav", "begränsning"]
concepts_used: ["teknik", "teknisk lösning", "behov"]
figures: ["fig-2.1.1-a", "fig-2.1.2-a"]
prerequisites: ["1.1"]
---

## Tekniska problem

Den som köper en borrmaskin vill egentligen inte ha en borrmaskin, utan ett hål i väggen, och inte ens hålet är målet, utan en uppsatt bokhylla. Att hålla isär vad någon vill uppnå och hur det ska gå till är kärnan i allt tekniskt arbete, och det kräver tre ord med skarpa betydelser.

Ett behov är det tillstånd någon vill uppnå, till exempel en bokhylla på väggen, rent vatten i kranen eller att komma torr till skolan. Ett *tekniskt problem* är hindret mellan nuläget och behovet, formulerat så att det går att angripa med teknik. Den [[begrepp:teknisk lösning]] som undanröjer hindret är svaret på problemet, inte på behovet direkt. I ett problemlösningsarbete går kedjan samma väg. Behovet pekar ut målet, problemet ringar in hindret och lösningen tar bort det. En ny teknik kan också växa fram ur en möjlighet som ännu inte har något behov, men även då får den betydelse först när den kopplas till ett behov eller problem.

### Många svar, olika bra

En matematikuppgift har ett facit. Ett tekniskt problem har det nästan aldrig. Behovet att ta sig över ett vatten kan mötas med en bro, en tunnel, en färja eller ett vadställe. Över Öresund kombinerades bron och tunneln med en konstgjord ö emellan. Lösningarna är inte rätt eller fel utan olika bra, beroende på vad de måste klara, vad de får kosta och vem de är till för. Att ett problem har många möjliga svar är inte en brist utan själva kännetecknet på ett tekniskt problem, och det är därför tekniskt arbete kräver både analys och beslut.

### Problemformuleringen styr lösningen

Cykelhjälmen skyddar huvudet, men många låter den hänga hemma. Den är i vägen, plattar frisyren och glöms bort. Två industridesignstudenter vid Lunds universitet, Anna Haupt och Terese Alstin, tog sig an saken i sitt examensarbete 2005. De formulerade inte problemet som att göra hjälmen bekvämare. De frågade i stället hur huvudet kan skyddas utan att cyklisten bär något på det. Svaret blev Hövding, en krage som läser av cyklistens rörelser och blåser upp en airbag runt huvudet i olycksögonblicket. Samma behov, en annan problemformulering, en helt annan lösning.

[BILD 2.1-1] Innehåll: Förgrenad kedja med Hövdingfallet: behovet att skydda cyklistens huvud överst, därunder två problemformuleringar sida vid sida (gör hjälmen bekvämare respektive skydda huvudet utan att cyklisten bär något på det), och under varje formulering dess lösning: en förbättrad cykelhjälm respektive airbagkragen. Kedjans tre nivåer märks behov, tekniskt problem och teknisk lösning. Bildtext: "Samma behov, två olika problemformuleringar, två helt olika lösningar. Problemformuleringen är en vägkorsning, inte en formalitet."

Exemplet visar varför problemformuleringen aldrig är en formalitet. Den vanligaste fällan är att baka in lösningen i problemet. Den som säger att skolan behöver fler mobilladdare har redan bestämt sig för laddare, fast behovet kanske är laddade mobiler och problemet lika gärna kan angripas med lånebatterier eller laddskåp. Ett väl formulerat problem nämner ingen lösning. Det beskriver hindret och lämnar vägarna öppna.

## Problemformulering

År 1956 skulle en av IKEA:s medarbetare få in ett bord i en bil. Det gick inte, så han sågade av benen och lade dem under bordsskivan. Ur den irritationen växte det platta paketet, principen som gjorde att möbler kunde fraktas och säljas till en bråkdel av kostnaden. Men för att göra platta paket av en hel möbelkatalog räcker ingen såg. Då krävs att problemet formuleras så exakt att konstruktörer kan arbeta efter det.

Vägen från behov till teknisk lösning går genom problemformuleringen, och en användbar formulering har tre delar. *Funktionen* beskriver vad lösningen ska göra, aldrig hur. Bordet ska till exempel bära sin last och kunna sättas ihop av kunden. *Krav* är villkor som lösningen måste uppfylla och som går att pröva, till exempel att skivan ska bära en viss vikt, att monteringen ska klara sig utan specialverktyg och att paketet ska rymmas i en personbil. *Begränsningar* är ramarna som inte får överskridas, som högsta tillverkningskostnad, valt material och gällande säkerhetsregler. Ett [[begrepp:tekniskt problem]] är färdigformulerat först när alla tre delarna är på plats.

[BILD 2.1-2] Innehåll: En komplett problemformulering för ett möbelexempel uppställd i tre fält: funktionen (vad lösningen ska göra, formulerad utan att nämna någon lösning), kraven (prövbara villkor som måste uppfyllas, t.ex. bärförmåga och montering utan verktyg) och begränsningarna (ramar som kostnad, material och fraktvolym). Pilar visar att kraven prövar funktionen och att begränsningarna ramar in båda. Bildtext: "En problemformulering är färdig först när funktion, krav och begränsningar alla är på plats."

### Vad, aldrig hur

Den vanligaste nybörjarmissen är att beskriva funktionen som en lösning, till exempel att bordet ska ha fyra avskruvbara ben. Då är beslutet redan fattat, och alla andra vägar är stängda, precis som när en lösning bakas in i själva problemformuleringen. Formuleringen bordet ska kunna transporteras platt och monteras av kunden lämnar öppet om benen skruvas, viks eller kilas fast. Funktionen pekar ut målet, och konstruktionsarbetet får hitta vägen.

### Krav som går att pröva

Ett krav gör bara nytta om det går att avgöra när det är uppfyllt. Skivan ska bära minst 50 kilogram går att pröva. Bordet ska vara stabilt går inte, förrän någon bestämt vad stabilt betyder. Otydliga krav skjuter bara upp konflikten till senare, när åsikterna går isär om lösningen duger. Antalet spelar också roll. Varje krav kostar konstruktionsfrihet, och den som kräver allt får oftast en lösning som är medelmåttig på allt.

### Begränsningar skapar lösningar

Begränsningar känns som hinder men fungerar ofta tvärtom. Utan bilens bagageutrymme hade ingen sågat av bordsbenen. Det var just den snäva ramen som pekade ut den nya lösningen. En formulering utan begränsningar bjuder in lösningar som aldrig kan byggas, och ett problem utan ramar är oftast för stort för att angripas alls. Den som får ett omöjligt problem bör därför pröva att skärpa ramarna i stället för att lätta på dem.

## Instuderingsfrågor

1. Vad är sambandet mellan behov, tekniskt problem och teknisk lösning?
2. Varför har ett tekniskt problem nästan aldrig bara ett rätt svar, till skillnad från en matematikuppgift?
3. Vilka tre delar ska en problemformulering innehålla?
4. Vad beskriver funktionen i en problemformulering?
5. Varför ska funktionen beskriva vad lösningen ska göra och inte hur den ser ut?
6. Vad krävs för att ett krav ska vara till nytta?
7. Vad är en begränsning i en problemformulering?
8. Varför kan en begränsning vara till hjälp snarare än ett hinder? Ge ett verkligt exempel.

## Övningar

1. **Behov, problem eller lösning?** Här är sex påståenden: (a) eleverna blir blöta på väg till cykelstället, (b) skolan behöver ett tak över cykelstället, (c) cyklarna välter i blåst, (d) skolan behöver fler mobilladdare, (e) matsalen är för högljudd vid lunch, (f) matsalen behöver ljudabsorbenter i taket. Sortera varje påstående som behov, tekniskt problem eller inbakad lösning. Skriv därefter om de påståenden som bakar in en lösning så att de blir tekniska problem som lämnar vägarna öppna.

2. **Gör kraven prövbara.** En grupp har skrivit dessa fem krav på en ny skolbänk. Bänken ska vara stabil, den ska tåla hård användning, den ska vara lätt att flytta, den ska hålla länge och den ska se bra ut. Skriv om vart och ett så att det går att avgöra när kravet är uppfyllt, och ange för varje omskrivet krav hur du skulle pröva det i praktiken. Avgör sist vilket av de fem krav som inte går att göra prövbart, och förklara varför.

3. **Begränsningarna bakom en produkt.** Välj en vardagsprodukt du har tillgång till, till exempel en hopfällbar sittdyna, en termos eller ett pennfodral. Undersök produkten och skriv ned vilka begränsningar som rimligen har styrt utformningen, som mått, material, tillverkningskostnad, vikt eller regler. Ändra därefter en av begränsningarna kraftigt, till exempel att produkten ska rymmas i en jackficka eller kosta hälften, och skissa hur lösningen då skulle behöva se ut. Förklara vilket krav som blev svårast att hålla efter ändringen.
