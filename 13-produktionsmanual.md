...
| `npm run validate` | Kör samtliga tvärgående kontroller. Standardläget skriver ut sammanfattning, varningar, migreringsskuld och fel; `-- --full` lägger till status- och täckningsöversikt (se "Uppföljning"). Körs också automatiskt före varje webbygge. |
...
Från denna status ställer `npm run validate` innehållskrav på avsnittet (målmodell, se "Migreringsläge"). Den fullständiga, gällande listan — frågespann, övningsspann, obligatoriska fält, förbjudna rubriker och formuleringar — definieras i 03-bokens-arkitektur.md och CLAUDE.md ("Hårda regler") snarare än upprepas här, så att en avskrift aldrig kan driva isär från den gällande regeln. Rätta alla fel och relevanta varningar. Jämför alltid mot referensimplementationen 1.1 innan status höjs.
...
**Övningar hör till sitt avsnitt.** Varje avsnitts `## Övningar` tränar och hänvisar bara till det egna avsnittets stoff. Utanför kapitel 11 har boken inga helkapitelövningar. I kapitel 11 är det elevdrivna projektet ett tillåtet tillägg vid sidan av avsnittsövningarna, som står kvar oförändrade i antal och krav (redaktionellt beslut 2026-07-29, som ersätter besluten 2026-07-24 och 2026-07-26 i den delen). Varje övning fungerar fristående (05, "Självreferenser"): ersätt "från föregående avsnitt" och "figuren ovan" med stabil information i uppgiften, och har ingen utskriven nivåstapling ("Bygg ut"/"Bygg ut vidare" utgår).
...
`npm run validate -- --full` är produktionens lägesrapport (`kapitel-klar` kör detta läge):
